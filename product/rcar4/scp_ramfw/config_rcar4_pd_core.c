...
static const char *core_pd_name_table[RCAR4_CORE_PER_CLUSTER_MAX] = {
...
static struct mod_rcar4_pd_core_config rcar4_pd_core_config_data = {
    .pd_notification_id = FWK_ID_NOTIFICATION_INIT(
        FWK_MODULE_IDX_POWER_DOMAIN,
        MOD_PD_NOTIFICATION_IDX_POWER_STATE_TRANSITION),
};

static struct fwk_element rcar4_pd_core_system_element_table[] = {
    [0] =
        {
            .name = "SYS0",
            .data = &((struct mod_rcar4_pd_core_pd_config){
                .pd_type = RCAR4_PD_TYPE_SYSTEM,
...
static const struct fwk_element *rcar4_pd_core_get_element_table(fwk_id_t mod)
{
    struct fwk_element *element_table, *element;
    struct mod_rcar4_pd_core_pd_config *pd_config_table, *pd_config;
    ...
    element_table = fwk_mm_calloc(
        core_count + cluster_count +
            FWK_ARRAY_SIZE(rcar4_pd_core_system_element_table) + 1,
        sizeof(struct fwk_element));
    if (element_table == NULL)
        return NULL;

    pd_config_table = fwk_mm_calloc(
        core_count + cluster_count, sizeof(struct mod_rcar4_pd_core_pd_config));
    ...
    for (core_idx = 0; core_idx < core_count; core_idx++) {
        ...
        pd_config->pd_type = RCAR4_PD_TYPE_CORE;
        pd_config->cluster_id =
            FWK_ID_ELEMENT(FWK_MODULE_IDX_RCAR4_PD_CORE, cluster_idx);
        pd_config->observer_id = FWK_ID_NONE;
        if (core_idx == RCAR4_PD_CORE_ELEMENT_IDX_CPU0)
    ...
    for (cluster_idx = 0; cluster_idx < cluster_count; cluster_idx++) {
        ...
        element->data = pd_config;
        pd_config->pd_type = RCAR4_PD_TYPE_CLUSTER;
        pd_config->observer_id = FWK_ID_NONE;
        pd_config->observer_api = FWK_ID_NONE;
        if (cluster_idx == RCAR4_PD_CORE_ELEMENT_IDX_CLU0)
            pd_config->always_on = true;
        else
            pd_config->always_on = false;
    ...
    memcpy(
        ...
        rcar4_pd_core_system_element_table,
        sizeof(rcar4_pd_core_system_element_table));
     ...
     */
    rcar4_pd_core_config_data.pd_source_id = fwk_id_build_element_id(
 ...
 */
const struct fwk_module_config config_rcar4_pd_core = {
    .elements = FWK_MODULE_DYNAMIC_ELEMENTS(rcar4_pd_core_get_element_table),
    .data = &rcar4_pd_core_config_data,
};
