...
static const struct op_points rcar4_h3_a57_op_points[EFUSE_AVS_NUM]
                                                   [NR_H3_A57_OPP] = {
                                                       {
                                                           {
                                                               500000000,
...
static const struct op_points rcar4_h3_a53_op_points[NR_H3_A53_OPP] = {
    {
        800000000,
        820000,
    },
...
static const struct op_points rcar4_m3_a57_op_points[EFUSE_AVS_NUM]
                                                   [NR_M3_A57_OPP] = {
                                                       {
                                                           {
                                                               500000000,
...
static const struct op_points rcar4_m3_a53_op_points[NR_M3_A53_OPP] = {
    {
        800000000,
        820000,
    },
...
uint32_t rcar4_dvfs_get_opp_voltage(int domain, int oppnr)
{
...
uint32_t rcar4_dvfs_get_opp_frequency(int domain, int oppnr)
{
    if (domain == A57_DOMAIN) {
        if (oppnr < 0 || oppnr >= current_a57_opp_limit)
            return ~0;
...
static unsigned long pll_clk_parent_rate(void)
{
    ...
    idx = (fwk_mmio_read_32(RCAR4_MODEMR) & MODEMR_BOOT_PLL_MASK) >>
...
/* TODO : Not supported because it conflicts with I2C control from the kernel */
    ...
    ret = rcar4_iic_dvfs_send(SUBORDINATE_ADDR_PMIC, REG_ADDR_DVFS_SetVID, val);
    if (ret) {
        return ret;
    }
    return 0;
...
static unsigned long get_voltage(void)
{
    ...
    ret =
      rcar4_iic_dvfs_receive(SUBORDINATE_ADDR_PMIC, REG_ADDR_DVFS_SetVID, &val);
    if (ret) {
        return ret;
    }
...
int rcar4_dvfs_get_nr_opp(int domain)
{
    if (domain == A57_DOMAIN)
...
int rcar4_dvfs_opp_init(void)
{
    ...
    product = fwk_mmio_read_32(RCAR4_PRR) & RCAR4_PRODUCT_MASK;

    if (product == RCAR4_PRODUCT_H3) {
        current_a57_opp_limit = ARRAY_SIZE(rcar4_h3_a57_op_points[efuse_avs]);
        current_a57_opp_table = &rcar4_h3_a57_op_points[efuse_avs][0];

        current_a53_opp_limit = ARRAY_SIZE(rcar4_h3_a53_op_points);
        current_a53_opp_table = &rcar4_h3_a53_op_points[0];
    } else if (product == RCAR4_PRODUCT_M3) {
        current_a57_opp_limit = ARRAY_SIZE(rcar4_m3_a57_op_points[efuse_avs]);
        current_a57_opp_table = &rcar4_m3_a57_op_points[efuse_avs][0];

        current_a53_opp_limit = ARRAY_SIZE(rcar4_m3_a53_op_points);
        current_a53_opp_table = &rcar4_m3_a53_op_points[0];
    } else
        return -1;
...
static int do_rcar4_clock_set_rate(
    fwk_id_t dev_id,
    uint64_t rate,
    enum mod_clock_round_mode round_mode)
{
    ...
    case MOD_RCAR4_CLOCK_A57_DIVIDER_DIV_EXT:
        ret = set_a57_opp(rate);
        break;
    case MOD_RCAR4_CLOCK_A53_DIVIDER_DIV_EXT:
...
static int rcar4_clock_set_rate(
    fwk_id_t dev_id,
    uint64_t rate,
    enum mod_clock_round_mode round_mode)
{
    ...
    return do_rcar4_clock_set_rate(dev_id, rate, round_mode);
}

static int rcar4_clock_get_rate(fwk_id_t dev_id, uint64_t *rate)
{
    struct rcar4_clock_dev_ctx *ctx;
...
static const struct mod_rcar4_clock_drv_api api_clock = {
    .set_rate = rcar4_clock_set_rate,
    .get_rate = rcar4_clock_get_rate,
};
    ...
    const void *data)
{
    ...
    module_ctx.dev_ctx_table =
        fwk_mm_calloc(element_count, sizeof(struct rcar4_clock_dev_ctx));
...
static int rcar4_clock_element_init(
    fwk_id_t element_id,
...
static int rcar4_clock_process_bind_request(
    fwk_id_t source_id,
    fwk_id_t target_id,
    fwk_id_t api_id,
    const void **api)
...
static int rcar4_clock_start(fwk_id_t id)
{
    /* for Not Module */
...
const struct fwk_module module_rcar4_clock = {
    ...
    .process_bind_request = rcar4_clock_process_bind_request,
    .start = rcar4_clock_start,
};
