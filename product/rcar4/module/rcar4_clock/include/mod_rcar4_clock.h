...
/*!
 * \defgroup GroupRCAR4Clock Clock
 * @{
 */
...
enum mod_rcar4_clock_api_type {
    ...
    MOD_RCAR4_CLOCK_API_TYPE_CSS,
    MOD_RCAR4_CLOCK_API_TYPE_SYSTEM,
    MOD_RCAR4_CLOCK_API_COUNT,
};

/*!
 * \brief Sub-types of rcar4 clock.
 */
enum mod_rcar4_clock_type {
    /*! A clock with a fixed source. Only its divider can be changed. */
    MOD_RCAR4_CLOCK_TYPE_SINGLE_SOURCE,
    /*! A clock with multiple, selectable sources and at least one divider. */
    MOD_RCAR4_CLOCK_TYPE_MULTI_SOURCE,
    /*!
     * A clock with multiple, selectable sources, at least one divider, and
     * support for modulation.
     */
    MOD_RCAR4_CLOCK_TYPE_CLUSTER,
};

/*!
 * \brief Divider register types.
 */
enum mod_rcar4_clock_msclock_divider {
    /*! Divider affecting the A57 PLL clock source. */
    MOD_RCAR4_CLOCK_A57_DIVIDER_DIV_EXT = 1,
    /*! Divider affecting the A53 PLL clock source. */
    MOD_RCAR4_CLOCK_A53_DIVIDER_DIV_EXT = 2,
};
...
enum mod_clusclock_source {
    ...
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL0 = 0,
    /*! The clock source is set to a private cluster PLL */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL1 = 1,
    /*! The clock source is set to a private cluster PLL */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL2 = 2,
    /*! The clock source is set to a private cluster PLL */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL3 = 3,
    /*! The clock source is set to a private cluster PLL */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL4 = 4,
    /*! The clock source is set to a private cluster PLL */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL5 = 5,
    /*! The clock source is set to a private cluster PLL */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL6 = 6,
    /*! The clock source is set to a private cluster PLL */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_PLL7 = 7,
    /*! Number of valid clock sources */
    MOD_RCAR4_CLOCK_CLUSCLK_SOURCE_MAX
};

/*!
 * \brief Rate lookup entry.
 */
struct mod_rcar4_clock_rate {
    /*! Rate in Hertz. */
    uint64_t rate;
    /*! Clock source used to obtain the rate (multi-source clocks only). */
    uint8_t source;
    /*! The divider register to use (multi-source clocks only). */
    enum mod_rcar4_clock_msclock_divider divider_reg;
 ...
 */
struct mod_rcar4_clock_dev_config {
    /*! The type of the clock device. */
    enum mod_rcar4_clock_type type;
    ...
    const struct mod_rcar4_clock_rate *rate_table;
...
/* Device context */
struct rcar4_clock_dev_ctx {
    bool initialized;
    uint64_t current_rate;
    enum mod_clock_state current_state;
    const struct mod_rcar4_clock_dev_config *config;
};

/* Module context */
struct rcar4_clock_ctx {
    struct rcar4_clock_dev_ctx *dev_ctx_table;
...
extern int32_t rcar4_iic_dvfs_receive(
    uint8_t subordinate,
    uint8_t reg,
    uint8_t *data);
extern int32_t rcar4_iic_dvfs_send(
    uint8_t subordinate,
    uint8_t regr,
    uint8_t data);
